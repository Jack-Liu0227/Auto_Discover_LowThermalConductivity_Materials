...
def get_high_symmetry_config(config: dict[str, Any] | None = None) -> dict[str, Any]:
    config = config or load_config()
    defaults = {
        "enabled": True,
        "min_crystal_system": "orthorhombic",
        "scope": "historical_seed_only",
        "parent_count": 10,
    }
    section = dict(config.get("high_symmetry", {}))
    # Accept the flat names used by older parameter sheets while keeping the
    # nested section as the canonical representation.
    if "high_symmetry_parent_enabled" in config:
        section.setdefault("enabled", config["high_symmetry_parent_enabled"])
    if "high_symmetry_min_crystal_system" in config:
        section.setdefault("min_crystal_system", config["high_symmetry_min_crystal_system"])
    if "high_symmetry_parent_scope" in config:
        section.setdefault("scope", config["high_symmetry_parent_scope"])
    return defaults | section


def get_llm_formula_generation_config(config: dict[str, Any] | None = None) -> dict[str, Any]:
    config = config or load_config()
    defaults = {"enabled": True, "start_iteration": 2, "proposal_count": 20, "parent_count": 10}
    section = dict(config.get("llm_formula_generation", {}))
    aliases = {
        "enabled": "llm_formula_generation_enabled",
        "start_iteration": "llm_formula_generation_start_iteration",
        "proposal_count": "llm_formula_proposal_count",
        "parent_count": "llm_formula_parent_count",
        "max_tokens": "llm_formula_generation_max_tokens",
        "max_retries": "llm_formula_generation_max_retries",
    }
    for field, alias in aliases.items():
        if alias in config:
            section.setdefault(field, config[alias])
    return defaults | section


def get_screening_config(config: dict[str, Any] | None = None) -> dict[str, Any]:
    config = config or load_config()
    defaults = {
        "chemistry_diversity_enabled": True,
        "evaluator_evidence": "chemistry_only",
        "use_candidate_ids": True,
        "max_same_element_tuple": 2,
        "max_same_stoichiometry_pattern": 4,
    }
    return defaults | dict(config.get("screening", {}))
...
def validate_theory_doc_sync(
    ...
    doc_set = set(doc_elements)
    config_set = set(prior["allowed_elements"])
    if doc_elements and doc_set != config_set:
        issues.append(
            "Element library mismatch: "
            f"doc={sorted(doc_set)} vs config={sorted(config_set)}"
        )
