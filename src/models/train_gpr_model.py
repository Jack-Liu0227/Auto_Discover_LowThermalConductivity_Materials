# -*- coding: utf-8 -*-
"""Train a Gaussian Process Regressor for thermal conductivity prediction."""
...
def load_processed_data(data_path):
    """Load composition features and target values."""
...
def plot_cross_validation(cv_scores, save_path):
    ...
    plt.axhline(
        ...
        linestyle="--",
        linewidth=2,
        label=f"Mean R^2 = {mean_score:.4f}",
    )

    plt.xlabel("Fold Number", fontsize=12, fontweight="bold")
    plt.ylabel("R^2 Score", fontsize=12, fontweight="bold")
...
def plot_prediction(y_true, y_pred, save_path, title=None):
    ...
    textstr = "\n".join((f"$R^2 = {r2:.4f}$", f"RMSE = {rmse:.4f}", f"MAE = {mae:.4f}"))
...
def train_gpr_model(data_path, output_dir):
    """Train and save the best GPR model found by 10-fold CV."""
    print("=" * 60)
    ...
    try:
        X, y, _feature_names = load_processed_data(data_path)
    ...
    X_trainval, X_test, y_trainval, y_test = train_test_split(X, y_log, test_size=0.1, random_state=42)
    ...
    kernel = (
        ConstantKernel(1.0) * RBF(length_scale=1.0) * RationalQuadratic(alpha=1.0, length_scale=1.0)
    ...
    print(f"\nAverage CV R^2: {np.mean(cv_scores):.4f} (+/- {np.std(cv_scores):.4f})")
    print(f"Best Fold: {best_fold_idx} (R^2 = {best_fold_score:.4f})")
    print(f"NOTE: Saving the model from fold {best_fold_idx}; no retraining on full data.")
    ...
    y_pred_log, _y_std = best_model.predict(X_test_scaled, return_std=True)
    ...
    meta_info = {
        ...
        "n_samples_trained_on": len(X_trainval) * 0.9,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input", type=str, default=default_input, help=f"Path to input CSV file (default: {default_input})")
    parser.add_argument("--output", type=str, default=default_output, help=f"Path to output directory (default: {default_output})")
