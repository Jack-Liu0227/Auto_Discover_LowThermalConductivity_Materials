...
class InvalidTheoryDocumentError(RuntimeError):
    """Raised when an LLM returns a truncated or structurally invalid document."""
...
def validate_updated_theory_document(updated_doc: str, original_doc: str) -> None:
    """Validate a complete English theory document before replacing any artifact."""
    text = str(updated_doc or "").strip()
    if not text:
        raise InvalidTheoryDocumentError("LLM theory update returned an empty document")
    if text.startswith("```"):
        raise InvalidTheoryDocumentError("LLM theory update returned a markdown wrapper instead of a document")
    if CJK_PATTERN.search(text):
        raise InvalidTheoryDocumentError("LLM theory update contains CJK characters")
    if text.count("```") % 2:
        raise InvalidTheoryDocumentError("LLM theory update contains an unclosed code fence and appears truncated")
    if not text.startswith("# "):
        raise InvalidTheoryDocumentError("LLM theory update does not start with a Markdown title")

    required_headings = re.findall(r"^## \\d+\\..+$", original_doc, flags=re.MULTILINE)
    missing_headings = [heading for heading in required_headings if heading not in text]
    if missing_headings:
        raise InvalidTheoryDocumentError(
            "LLM theory update is structurally incomplete; missing headings: "
            + "; ".join(missing_headings[:4])
        )
    dynamic_headings = [
        heading
        for heading in re.findall(r"^### \\d+\\.\\d+ .+$", original_doc, flags=re.MULTILINE)
        if "Success Case Theory Summary" in heading or "Theory Optimization Record" in heading
    ]
    missing_dynamic = [heading for heading in dynamic_headings if heading not in text]
    if missing_dynamic:
        raise InvalidTheoryDocumentError(
            "LLM theory update is missing required dynamic sections: " + "; ".join(missing_dynamic)
        )

    minimum_length = max(200, int(len(original_doc) * 0.60))
    if len(text) < minimum_length:
        raise InvalidTheoryDocumentError(
            f"LLM theory update is suspiciously short ({len(text)} chars; expected at least {minimum_length})"
        )
    if text.endswith((":", ",", "-", "{", "[")):
        raise InvalidTheoryDocumentError("LLM theory update ends mid-structure and appears truncated")
    ...
    def update_theory_document(
        ...
        if not Path(success_csv).exists():
            ...
            if resolved_original_doc_path.resolve() != doc_file.resolve():
                _atomic_write_text(doc_file, original_content)
        ...
        try:
            updated_doc = self.ai_client.chat(
                ...
                max_tokens=self.ai_client.get_default_max_tokens("theory_update"),
                auto_fallback=False,
            )
        except Exception as e:
            ...
            invalid_file = self._save_invalid_output(
                ...
                iteration_num,
                results_root,
                error="LLM theory update request failed",
            )
            logger.info(f"Invalid theory update diagnostic saved: {invalid_file}")
            return None
        
        # 7. Validate and save LLM output only after the complete-document contract passes.
        try:
            validate_updated_theory_document(updated_doc, original_doc)
        except InvalidTheoryDocumentError as exc:
            invalid_file = self._save_invalid_output(
                updated_doc,
                iteration_num,
                results_root,
            ...
            logger.error("Rejecting invalid theory document: %s", exc)
            logger.error("Invalid theory output preserved at: %s", invalid_file)
            return None
        ...
        _atomic_write_text(doc_file, updated_doc)
...
8. Treat every prototype and element family as a testable hypothesis. Do not convert repeated observations from one high-symmetry family into a universal rule or a hard filter for future formulas.
9. Preserve and explicitly discuss chemically plausible alternatives when the current successful set is narrow; a lack of observed success is not evidence of chemical impossibility.
10. Maintain the existing document structure and output the full Markdown document only.
"""
        prompt += """
...
"""
    ...
    def _save_input(self, prompt: str, iteration_num: int, results_root: str = "results") -> str:
        ...
        _atomic_write_text(input_file, prompt)
    ...
    def _save_output(self, response: str, iteration_num: int, results_root: str = "results") -> str:
        ...
        _atomic_write_text(output_file, f"{str(response).rstrip()}\n")
    ...
    def _save_invalid_output(
        ...
        """Preserve rejected output without making it a resumable artifact."""
        output_dir = Path(f"{results_root}/iteration_{iteration_num}/reports")
        invalid_file = output_dir / "llm_theory_update_output.invalid.md"
        content = (
            "# Invalid Theory Document Update\n\n"
            f"**Error**: {error}\n\n"
            "## Raw model response\n\n"
            f"{str(response or '').rstrip()}\n"
        )
