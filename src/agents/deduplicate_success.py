...
def deduplicate_success_materials(
    ...
    def load_structure_from_row(row, csv_dir):
        ...
        if rel_path is not None:
            ...
            for candidate in (Path(csv_dir) / str(rel_path), Path.cwd() / str(rel_path)):
                if candidate.exists():
                    try:
    ...
    # Preserve rows whose structure could not be parsed instead of silently dropping them.
    parsed_indices = {idx for idx, _ in structures}
    unique_indices.extend(sorted(set(df.index) - parsed_indices))
    df_dedup = df.loc[sorted(set(unique_indices))].copy()
