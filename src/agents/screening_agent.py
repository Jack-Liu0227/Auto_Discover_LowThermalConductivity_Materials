...
def rank_by_ei(candidates: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Rank candidates by EI while tolerating missing LLM-side predictions."""
    def _safe_ei(candidate: dict[str, Any]) -> float:
        value = candidate.get("ei", candidate.get("score", 0.0))
        try:
            parsed = float(value)
    ...
    return sorted(candidates, key=lambda item: -_safe_ei(item))
