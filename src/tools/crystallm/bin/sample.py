...
class SampleDefaults:
    ...
    generate_dir: str = "generated_structures"  # the path to the directory to store the generated structures
...
if __name__ == "__main__":
    ...
    model.eval()
    model.to(C.device)
    # Disable compilation due to PyTorch compatibility issues
    # if C.compile:
        ...
        with ctx:
            for k in range(C.num_samples):
                ...
                if C.target == "console":
                    ...
                    with open(os.path.join(C.generate_dir, fname), "wt") as f:
                        f.write(generated)
