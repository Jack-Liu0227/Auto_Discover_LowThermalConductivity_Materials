...
def generate(model_dir, seed, device, dtype, num_gens, temperature, top_k, max_new_tokens, chunk_of_prompts, queue):
    """
    Generate CIF files using the trained model.
    ...
    Args:
        ...
        num_gens (int): Number of generations per prompt
        temperature (float): Sampling temperature
        top_k (int): Top-k sampling parameter
        max_new_tokens (int): Maximum number of tokens to generate
        chunk_of_prompts (list): List of (id, prompt) tuples
        queue (multiprocessing.Queue): Queue for progress tracking
        
    Returns:
        list: List of generated (id, structures) tuples
    """
    ...
    with torch.no_grad():
        with ctx:
            for id, prompt in chunk_of_prompts:
                ...
                gens = generate_structures(model, x, num_gens, max_new_tokens, temperature, top_k, decode)
...
def load_model(model_dir, device):
    """Helper function to load the model from checkpoint"""
...
def prepare_model(model, device):
    """Helper function to prepare model for inference"""
    model.eval()
    model.to(device)
    return torch.compile(model)  # requires PyTorch 2.0


def generate_structures(model, x, num_gens, max_new_tokens, temperature, top_k, decode):
    """Helper function to generate multiple structures"""
    return [
        decode(model.generate(x, max_new_tokens, temperature=temperature, top_k=top_k)[0].tolist())
