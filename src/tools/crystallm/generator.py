...
def generate_crystal_from_composition(
    ...
    try:
        ...
        if requested_device == "cpu":
            ...
            validate_gpu_devices([requested_device])
            set_process_cuda_device(requested_device)
            device = requested_device
            active_index = torch.cuda.current_device()
            logger.info(
                "Using CUDA device",
                device=device,
                gpu=torch.cuda.get_device_name(active_index),
            )
        else:
            raise ValueError(f"Unsupported CrystaLLM device: {device}")
        ...
        # Pipeline failure is a hard generation failure. Do not inspect or reuse
        # files left by an earlier attempt in the same output directory.
        if not pipeline_result.get("success", False):
            error = pipeline_result.get("message", "Unknown CrystaLLM pipeline error")
            logger.error("Pipeline failed", error=error)
            return {
                "success": False,
                "error": f"CrystaLLM pipeline failed: {error}",
                "composition": composition,
                "generation_id": generation_id,
            }
        ...
        if not cif_files:
            logger.error("CrystaLLM postprocessing produced no CIF files")
            return {
                "success": False,
