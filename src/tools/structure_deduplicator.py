...
class StructureDeduplicator:
    ...
    def deduplicate_multi_dirs(
        ...
        """Deduplicate independent formula subdirectories with bounded workers.

        Each formula directory owns its CIF files and CSV, so it can be handled
        by an isolated process. The parent only collects results and preserves
        the caller's formula order; no aggregate artifact is written here.
        """
        ...
        if formula_dirs is None:
            ...
            formula_dirs = list(formula_dirs)
        ...
        for formula in formula_dirs:
            ...
            if not comp_dir.exists():
                ...
                continue
            work_items.append((formula, comp_dir))
        ...
        if worker_count == 1:
            for formula, comp_dir in work_items:
                results.append(
                    _deduplicate_formula_worker(
                        (formula, str(comp_dir), keep_duplicates, matcher_config)
                    )
                )
        else:
            ordered: list[tuple[str, Dict[str, Any]] | None] = [None] * len(work_items)
            with ProcessPoolExecutor(max_workers=worker_count) as executor:
                futures = {
                    ...
                    for index, (formula, comp_dir) in enumerate(work_items)
                }
                for future in as_completed(futures):
                    ordered[futures[future]] = future.result()
            results = [item for item in ordered if item is not None]

        return {formula: result for formula, result in results}


def _dedup_failure_result(error: str) -> Dict[str, Any]:
    return {
        "total": 0,
        "unique": 0,
        "removed": 0,
...
def _deduplicate_formula_worker(
    payload: tuple[str, str, bool, Dict[str, Any]],
) -> tuple[str, Dict[str, Any]]:
    """Run one formula directory without sharing a matcher between workers."""
    formula, comp_dir, keep_duplicates, matcher_config = payload
    try:
        deduplicator = StructureDeduplicator(**matcher_config)
        result = deduplicator.deduplicate_structures(
            Path(comp_dir),
            keep_duplicates=keep_duplicates,
        )
    except Exception as exc:
        logger.exception("Deduplication failed for formula %s", formula)
        result = _dedup_failure_result(str(exc))
...
def deduplicate_relaxed_structures(
    ...
    return deduplicator.deduplicate_multi_dirs(
        relax_dir,
        formulas,
        keep_duplicates,
