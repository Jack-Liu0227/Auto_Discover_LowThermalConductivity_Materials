...
try:
    ...
    from ..utils.gpu_parallel import set_process_cuda_device, validate_gpu_devices
...
class MattersimWrapper(BaseTool):
    ...
    def __init__(
        ...
        if config:
            ...
            self.requested_device = config.get('device')
        ...
        # Respect an explicit device from the worker; otherwise auto-detect.
        self.device = self._detect_device(self.requested_device)

    def _detect_device(self, requested_device: Optional[str] = None) -> str:
        """Resolve and select an explicit logical CUDA device."""
        ...
        try:
            ...
            logger.warning("PyTorch unavailable; using CPU")
            return "cpu"

        cuda_available = bool(torch.cuda.is_available() and torch.cuda.device_count() > 0)
        if requested.startswith("cuda"):
            validate_gpu_devices([requested])
            set_process_cuda_device(requested)
            active_index = torch.cuda.current_device()
            logger.info(
                "✅ CUDA available, using %s: %s",
                requested,
                torch.cuda.get_device_name(active_index),
            )
            return requested
        if cuda_available:
            set_process_cuda_device("cuda")
            active_index = torch.cuda.current_device()
            logger.info(
                "✅ CUDA available, using cuda:%s: %s",
                active_index,
                torch.cuda.get_device_name(active_index),
            )
            return "cuda"
    ...
    def check_availability(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                importlib.import_module("mattersim.forcefield")
                importlib.import_module("mattersim.applications.phonon")
    ...
    def _calculate_with_mattersim(
        ...
        if calculate_phonon:
            try:
                ...
                if str(self.device).startswith("cuda"):
                    try:
                        ...
                        if torch.cuda.is_available():
                            ...
                            current_mem = torch.cuda.memory_allocated(active_device) / 1024**3
                            total_mem = torch.cuda.get_device_properties(active_device).total_memory / 1024**3
                ...
                # Keep rendered phonon images, but remove bulky/intermediate
                # phonopy YAML files.  Relaxed CIF/CSV outputs are written by
                # the worker and are not touched here.
                if work_dir:
                    try:
                        ...
                        for temp_file in ("phonopy_params.yaml", "band.yaml", "mesh.yaml"):
                            ...
                            if temp_path.exists():
                                temp_path.unlink()
                                logger.debug("Removed phonopy intermediate: %s", temp_path)
                    except Exception as cleanup_error:
                        logger.debug("Failed to clean phonopy YAML intermediates: %s", cleanup_error)
    ...
    def _cleanup_gpu_cache(self):
        ...
        try:
            ...
            if torch.cuda.is_available() and torch.cuda.device_count() > 0:
                ...
                # 3. Synchronize the selected CUDA device
                active_device = torch.device(self.device) if str(self.device).startswith("cuda") else None
    ...
    def _get_or_create_calculator(self):
        ...
        try:
            ...
            if str(self.device).startswith("cuda") and torch.cuda.is_available() and torch.cuda.device_count() > 0:
                active_device = torch.device(self.device)

                # 1. 清空 selected GPU cache
                torch.cuda.empty_cache()
                
                # 2. Synchronize the selected device
                torch.cuda.synchronize(active_device)
                ...
                try:
                    torch.cuda.reset_accumulated_memory_stats(active_device)
                except:
                    pass
    ...
    def _relax_structure_real(
        ...
        """使用指定设备执行 MatterSim relaxation；CUDA OOM 直接向外传播。"""
        ...
        try:
            ...
            if self._is_cuda_oom_error(exc):
                logger.warning(
                    "CUDA OOM during relaxation for %s; propagating for per-structure skip",
                    structure.composition.formula,
                )
            else:
