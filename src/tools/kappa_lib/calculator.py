...
class ThermalConductivityCalculator:
    ...
    def _prepare_data(self, device: Optional[str] = None) -> pd.DataFrame:
        """Prepare data for thermal conductivity calculation on the requested device."""
        try:
            ...
            predict.main(target_model_path, self.root_dir, device=device)
    ...
    def calculate_kappa_p(self, device: Optional[str] = None) -> pd.DataFrame:
        """
        ...
        try:
            ...
            df = self._prepare_data(device=device)
    ...
    def calculate_kappa_mtp(self, device: Optional[str] = None) -> pd.DataFrame:
        """
