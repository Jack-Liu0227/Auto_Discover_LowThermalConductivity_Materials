...
def main(model_path, root_dir, device=None):
    ...
    validate(test_loader, model, criterion, normalizer, test=True, device=run_device)


def validate(val_loader, model, criterion, normalizer, test=False, device=None):
    ...
    model.eval()
    run_device = device or _resolve_device(getattr(args, "device", None))

    end = time.time()
    for i, (input, target, batch_cif_ids) in enumerate(val_loader):
        with torch.no_grad():
            if args.cuda:
                input_var = (Variable(input[0].to(run_device, non_blocking=True)),
                             Variable(input[1].to(run_device, non_blocking=True)),
                             input[2].to(run_device, non_blocking=True),
                             [crys_idx.to(run_device, non_blocking=True) for crys_idx in input[3]])
            else:
        ...
        with torch.no_grad():
            if args.cuda:
                target_var = Variable(target_normed.to(run_device, non_blocking=True))
            else:
