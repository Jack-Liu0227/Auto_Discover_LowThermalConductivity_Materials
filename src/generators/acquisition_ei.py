...
from monte_carlo_sampler import (
    ...
    is_valid_hard_constrained_composition,
    ELEMENTS,
    MAX_ATOMS,
)
...
def _canonical_formula(value) -> str:
    """Return a stable formula key for candidate/history de-duplication."""
    ...
    try:
        ...
        return str(Composition(text).reduced_formula).replace(" ", "")
    except Exception:
        return "".join(text.split())


def _deduplicate_formula_candidates(
    samples: list[dict],
    excluded_formulas: set[str] | None = None,
) -> tuple[list[dict], int]:
    """Keep first-seen candidates without silently replenishing the pool."""
    seen = {
        _canonical_formula(formula)
        for formula in (excluded_formulas or set())
        if _canonical_formula(formula)
    }
    unique: list[dict] = []
    removed = 0
    for sample in samples:
        key = _canonical_formula(sample.get("formula"))
...
def _load_evaluated_formula_keys(
    iteration_num: int,
    project_root: str,
    results_root: str,
) -> set[str]:
    """Load formulas that already have evaluated thermal results."""
    historical = load_historical_success_data(iteration_num, project_root, results_root)
    if historical is None or "formula" not in historical.columns:
        return set()
    return {
        key
        for value in historical["formula"].tolist()
        if (key := _canonical_formula(value))
    }
...
def mutate_sampling(
    ...
    for s in initial_samples:
        formula_key = _canonical_formula(s.get('formula'))
        if formula_key:
            generated_formulas.add(formula_key)
    ...
    while len(mutated_samples) < n_mutations and attempts < max_attempts:
        ...
        try:
            ...
            new_formula = composition_to_formula(new_comp_dict)
            formula_key = _canonical_formula(new_formula)
            if formula_key and formula_key not in generated_formulas:
                generated_formulas.add(formula_key)
...
def main(
    ...
    if sampling_method is None:
        sampling_method = sampling_params.get("method", "random")
    if mcmc_params is None:
        mcmc_params = sampling_params.get("mcmc", {})
    hard_constraints = sampling_params.get("hard_constraints")

    sampling_method = (sampling_method or "random").lower()
    mcmc_params = mcmc_params or {}
    evaluated_formula_keys = _load_evaluated_formula_keys(
        iteration_num,
        project_root,
        results_root,
    )
    logger.info(
        "Historical evaluated candidates excluded from sampling: %d",
        len(evaluated_formula_keys),
    )
    #=========================================================================
    ...
    if sampling_method == "mcmc":
        ...
        samples = mcmc_sampling(
            ...
            excluded_formulas=evaluated_formula_keys,
        )
    else:
        if initial_samples and len(initial_samples) > 0:
            ...
            mutated_samples = mutate_sampling(
                initial_samples,
                n_mutations,
                logger,
                hard_constraints=hard_constraints,
    ...
    sampler_generated_count = len(samples)
    samples, duplicate_candidates_removed = _deduplicate_formula_candidates(
        samples,
        excluded_formulas=evaluated_formula_keys,
    )
    logger.info(
        "Candidate de-duplication: removed %d candidates; retained %d",
        duplicate_candidates_removed,
        len(samples),
    )
    if not samples:
        raise RuntimeError("BO candidate sampler returned no unevaluated candidates")
    ...
    all_sampled = list(samples)
    X = np.array([composition_to_features(s["composition"]) for s in all_sampled])
    X_scaled = scaler.transform(X)
    logger.info(f"特征矩阵形状: {X.shape}")

    # GPR预测 (返回均值和标准差)
    logger.info("GPR预测中...")
    all_mu_log, all_sigma_log = model.predict(X_scaled, return_std=True)

    # Every hard-constrained candidate participates in EI ranking.
    samples = all_sampled
    logger.info(
        "Candidate counts: requested=%d, sampler_generated=%d, candidate_pool=%d",
        int(n_samples),
        sampler_generated_count,
        len(samples),
    )

    accepted_indices = list(range(len(all_sampled)))
    mu_log = np.asarray([all_mu_log[index] for index in accepted_indices], dtype=float)
    sigma_log = np.asarray([all_sigma_log[index] for index in accepted_indices], dtype=float)
    for index, sample in enumerate(all_sampled):
        sample["k_pred"] = float(np.exp(all_mu_log[index]))
        sample["mu_log"] = float(all_mu_log[index])
        sample["sigma_log"] = float(all_sigma_log[index])
    if not samples:
        raise RuntimeError("BO candidate sampler returned no candidates after validation")
    ...
    raw_file = os.path.join(results_dir, 'raw_samples.csv')
    raw_columns = [
        'formula',
        'k_pred',
        'mu_log',
        ...
        [
            {
                ...
                'k_pred': sample.get('k_pred'),
                'mu_log': sample.get('mu_log'),
                'sigma_log': sample.get('sigma_log'),
                'ei': sample.get('ei'),
                'k_lower': sample.get('k_lower'),
                'k_upper': sample.get('k_upper'),
                'elements': ', '.join(sample.get('elements', [])),
                'n_elements': sample.get('n_elements', 0),
                'total_atoms': sample.get('total_atoms', 0),
            }
            for sample in all_sampled
        ],
        columns=raw_columns,
    ).to_csv(raw_file, index=False, encoding='utf-8-sig')
    ...
    sampling_status = (
        "completed"
        if len(all_sampled) >= int(n_samples) and len(samples) >= int(n_top)
    ...
    top_data = {
        ...
        'n_samples': n_samples,
        'sampler_generated_count': sampler_generated_count,
        'candidate_pool_count': len(all_sampled),
        'candidate_dedup_removed': duplicate_candidates_removed,
        'raw_sample_count': len(all_sampled),
        'n_top': n_top,
        'selected_count': min(len(samples_sorted), int(n_top)),
        'sampling_status': sampling_status,
        'raw_samples_file': raw_file,
        f'top{n_top}': []
    }
