...
def step_ai_evaluation(
    ...
    if path_config:
        resolved_results_root = str(path_config.results_root)
    else:
    ...
    try:
        ...
        selected_csv_dir = Path(resolved_results_root) / f"iteration_{iteration_num}" / "selected_results"
        ...
        temp_csv_path = selected_csv_path.with_name(f".{selected_csv_path.name}.{os.getpid()}.tmp")
        try:
            df_selected.to_csv(temp_csv_path, index=False, encoding="utf-8-sig")
            os.replace(temp_csv_path, selected_csv_path)
        finally:
