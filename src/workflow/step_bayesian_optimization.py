...
def step_bayesian_optimization(
    ...
    if path_config:
        ...
        resolved_models_root = str(path_config.models_root)
    ...
    try:
        ...
        sampling_metadata = {}
        metadata_path = (
            Path(resolved_results_root)
            / f"iteration_{iteration_num}"
            / "selected_results"
            / f"top{n_top}_materials.json"
        )
        try:
        ...
        return {
            "success": True,
            "status": sampling_metadata.get(
                "sampling_status",
                "completed" if len(results) >= n_top else "completed_with_partial_results",
            ),
