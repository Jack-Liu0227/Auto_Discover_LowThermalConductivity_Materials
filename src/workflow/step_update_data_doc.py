...
def step_update_data_and_doc(
    ...
    def _atomic_copy(source_path: Path, output_path: Path) -> None:
        output_path.parent.mkdir(parents=True, exist_ok=True)
        temp_path = output_path.with_name(f".{output_path.name}.{os.getpid()}.tmp")
        try:
    ...
    def _write_doc_update_error(output_path: Path, message: str) -> None:
        _atomic_write_text(output_path, message)

    def _write_doc_update_report(output_path: Path, title: str, body: str) -> None:
        _atomic_write_text(output_path, f"# {title}\n\n{body}\n")
    ...
    if has_success:
        ...
        # 检查 success_csv 是否有效
        if not success_csv:
            print("[ERROR] success_csv is missing; cannot complete dataset update")
            return {'success': False, 'error': 'Success materials CSV is missing'}
        else:
            # The dataset chain is strict: iteration N may only consume
            # iteration N-1, never an older fallback.
            prev_iteration = iteration_num - 1
            if path_config:
                data_dir_base = path_config.data_root
            else:
                data_dir_base = project_root / data_root
            origin_csv = data_dir_base / f"iteration_{prev_iteration}" / "data.csv"
            if not origin_csv.exists():
                print(f"[ERROR] Immediate previous dataset is missing: {origin_csv}")
                return {'success': False, 'error': f'Previous iteration data not found: {origin_csv}'}
            ...
            if updated_data_path:
                ...
                error = "Dataset update failed for success materials"
                print(f"[ERROR] {error}")
    ...
    if has_stable and not has_success:
        ...
        # 检查 stable_csv 是否有效
        if not stable_csv:
            print("[ERROR] stable_csv is missing; cannot complete dataset update")
            return {'success': False, 'error': 'Stable materials CSV is missing'}
        else:
            # The dataset chain is strict: iteration N may only consume
            # iteration N-1, never an older fallback.
            prev_iteration = iteration_num - 1
    ...
    if not has_success and not has_stable:
        ...
        source_data = data_dir_base / f"iteration_{prev_iteration}" / "data.csv"
        if source_data.exists():
            _atomic_copy(source_data, output_path)
            updated_data_path = str(output_path)
            print(f"[SUCCESS] Copied data: {source_data} -> {output_path}")
        else:
            print(f"[ERROR] Immediate previous dataset is missing: {source_data}")
            return {
                'success': False,
                'error': f'Previous iteration data not found: {source_data}',
            }
    ...
    if materials_csv:
        ...
        try:
            ...
            analysis_results_root = str(path_config.results_root) if path_config else results_root
            analysis_doc_root = str(path_config.doc_root) if path_config and path_config.doc_root else doc_root
            result = analyze_success_and_update_theory(
                success_csv_path=success_csv,
                iteration_num=iteration_num,
                version=version,
                results_root=analysis_results_root,
                doc_root=analysis_doc_root,
            )
    ...
    if updated_doc_path:
        ...
        if src_path != dst_path:
            _atomic_copy(Path(updated_doc_path), versioned_doc_path)
